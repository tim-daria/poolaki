...
/**
 * Rows as [date, type, category, name, amount]; category IDs follow
 * SEED_CATEGORIES and contributions carry none. More than one page, so
 * pagination can be exercised.
 */
const MOCK_ROWS: [string, EntryType, number | null, string, number][] = [
  ...
  ["2026-07-12", "expense", 4, "BVG monthly ticket", 49],
  ["2026-07-08", "expense", 6, "Pharmacy", 18.75],
  ...
  ["2026-06-20", "expense", 3, "Bookshop", 24.99],
...
export const MOCK_TRANSACTIONS: Transaction[] = MOCK_ROWS.map(
  ([transaction_date, entry_type, category, description, amount], i) => ({
    id: i + 1,
    entry_type,
    category,
    description,
