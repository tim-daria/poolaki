...
class ContextBuilder:
    ...
    def build_context(self, retrieval_result: CombinedRetrievalResult) -> list[FinancialContextItem]:
        """ Main method to create the context: choose sources & concatenates the contexts"""

        context = []

        if retrieval_result.financial_data:
            context.extend( self._build_financial_context(retrieval_result.financial_data))

        if retrieval_result.semantic_documents:
            context.extend(self._build_semantic_context(retrieval_result.semantic_documents))
    ...
    def _build_financial_context(self, data: dict):
        ...
        if ("current_month_expenses" in data
            and "previous_month_expenses" in data):
            ...
            items.append( FinancialContextItem( type="financial_summary",
                source="django_mock", content=f"""Expenses increased {increase:.0f}%.
        ...
        if "top_category" in data:
            ...
            items.append(FinancialContextItem(type="category_analysis", source="django_mock",
    ...
    def _build_semantic_context(self,documents:list[str]):
        ...
        return [FinancialContextItem(type="semantic_knowledge",
                source="vector_db", content=document) for document in documents]
