...
def test_build_financial_context():
    ...
    result = CombinedRetrievalResult(
        user_id=123,
        question="Why am I spending more?",
...
def test_add_semantic_context():
    ...
    result = CombinedRetrievalResult(
        user_id=123,
        question="What is food category?",
...
def test_context_size_limit():
    ...
    result = CombinedRetrievalResult(
        user_id=1, question="test", semantic_documents=["x" * 5000]
    )
...
def test_context_size_limit_does_not_exceed_max_length():
    result = CombinedRetrievalResult(
        user_id=1, question="test", semantic_documents=["a" * 3000, "b" * 3000]
    )
