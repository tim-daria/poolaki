...
from app.services.retrieval import MockRetriever, MockDocumentRepository
...
async def test_mock_retriever_returns_structured_format():
    ...
    mock_items = [
        RetrievedItem(type="financial_summary", content="Spent €850", source="django", metadata={}),
        RetrievedItem(type="category_breakdown", content="Food €230", source="django", metadata={})
    ]

    with patch.object(retriever, "get_context", new_callable=AsyncMock) as mock_get_context:
        mock_get_context.return_value = CombinedRetrievalResult(
            items=mock_items
        )
...
async def test_mock_retriever_top_k_limit():
    ...
    mock_items = [
        RetrievedItem(type="goal_progress", content="Goal 60%", source="django", metadata={})
    ]
